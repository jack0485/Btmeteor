#ifndef BTMETEOR_PEER_H
#define BTMETEOR_PEER_H
...
#define DATA                4   // 表明处于与peer交换数据状态
#define CLOSING             5   // 表明处于即将与peer断开状态

// 发送和接收缓冲区大小，16k可以存放一个slice，2K用来存放其他消息
...
typedef struct _Peer {
    ...
    unsigned int    up_total;                   // 从该peer上传的总字节数
...
Peer* add_peer_node();                          // 添加一个peer节点
int del_peer_node(Peer *peer);                  // 删除一个peer节点
void free_peer_node(Peer *node);                // 释放一个peer节点的内存
